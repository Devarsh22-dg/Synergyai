...
def ba_module():
    ...
    with tab3:
        ...
        context_text = st.text_area(
            ...
            value=prefill[:3000], height=150, key=f"doc_context_{cp}",
        )
        ...
        if st.button(f"Generate Draft {doc_type}", key=f"generate_doc_btn_{cp}"):
            if doc_type == "Data Dictionary":
                with st.spinner("Drafting data dictionary..."):
                    rows = generate_data_dictionary(context_text, user_suggestion)
                if rows:
                ...
                with st.spinner("Mapping As-Is and To-Be process steps..."):
                    rows = generate_asis_tobe(context_text, user_suggestion)
                if rows:
                    proj["documents_drafted"] = proj.get("documents_drafted", 0) + 1
    ...
    with tab4:
        ...
        source_text = st.text_area(
            ...
            value=proj.get("extracted_text", "")[:3000], height=150, key=f"story_source_{cp}",
        )
